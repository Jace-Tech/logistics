...
function listAllParcels()
{
  global $connect;
  try {
    $result = $connect->query("SELECT p.*, psd.id as psd_id, psd.service, psd.terms, psd.special_handling_section, psd.estimated_date FROM parcel p INNER JOIN parcel_shipment_details psd ON p.id = psd.parcel");
    $result->execute();
...
function getParcel(string $id)
{
  global $connect;
  try {
    $result = $connect->prepare("SELECT p.*, psd.id as psd_id, psd.service, psd.terms, psd.special_handling_section, psd.estimated_date FROM parcel p INNER JOIN parcel_shipment_details psd ON p.id = psd.parcel WHERE p.id = ?");
    $result->execute([$id]);
    return $result->fetch();
  } catch (Exception $e) {
    setAlert($e->getMessage());
